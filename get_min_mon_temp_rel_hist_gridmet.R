# This code downloads minimum mean monthly temperature relative to historic 
# across wbb range from 1998-2020. Data from GRIDMET.
# historic defined as mean of monthly min temperature from 1979-1997
...
hist_month <- ee$ImageCollection$fromImages(
  months$map(ee_utils_pyfunc(function(m) {
    return(hist_data$filter(ee$Filter$calendarRange(m, m, 'month'))$
             ...
             select('tmmn')$mean()$subtract(273.15)$
             set('month', m))
  })))
...
yearmap <- function(y) {
  ...
  # Set year id
  dataset <- dataset$set('out_name', ee$String('min_mon_temp_rel_hist_')$
...
task_img <- gt$batch$Export$imagecollection$toDrive(
  ...
  folder = 'gee_min_mon_temp_rel_hist',
